...
export default function AddCustomer() {
    ...
    const totalCost = entry.components.reduce((sum, c) => sum + (parseFloat(c.price) || 0), 0);
    ...
    const handleSubmit = async (e) => {
        ...
        if (validComponents.length === 0) { toast.error('Add at least one component/part'); return; }
        ...
        const payload = {
            ...
            serviceEntry: {
                ...entry,
                components: validComponents.map((c) => ({ name: c.name.trim(), price: parseFloat(c.price) || 0 })),
                totalCost,
        ...
        try {
            ...
            setSuccess(true);
            setTimeout(() => {
                setStep(1); setPhoneInput('');
                setForm({ name: '', phone: '', address: '' });
                setEntry({ ...EMPTY_ENTRY }); setSuccess(false);
                setExistingCustomer(null); setIsNew(false);
            }, 2000);
        } catch (err) {
            toast.error(err.message);
        } finally {
            ...
            <div className="add-page">
                ...
                <form onSubmit={handlePhoneCheck} className="phone-check-form">
                    <div className="form-field">
                        ...
                        <input type="tel" placeholder="e.g. 9876543210" value={phoneInput} onChange={(e) => setPhoneInput(e.target.value)} className="form-input" autoFocus />
            ...
            <form onSubmit={handleSubmit} className="customer-form">
                <div className="form-section">
                    ...
                    <div className="form-field">
                        ...
                        <input type="text" placeholder="Customer's full name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="form-input" readOnly={!!existingCustomer} />
                    </div>
                    <div className="form-field">
                        <label><Phone size={13} /> Phone</label>
                        <input type="tel" value={form.phone} className="form-input" readOnly />
                    </div>
                    <div className="form-field">
                        <label><MapPin size={13} /> Address *</label>
                        <textarea placeholder="Full address" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} className="form-textarea" rows={2} readOnly={!!existingCustomer} />
                    </div>
                </div>

                <div className="form-section">
                    <div className="section-title"><Wrench size={16} /> Work Details</div>
                    <div className="form-row">
                        <div className="form-field">
                            <label>Date *</label>
                            <input type="date" value={entry.date} onChange={(e) => setEntry({ ...entry, date: e.target.value })} className="form-input" />
                        </div>
                        <div className="form-field">
                            <label>Type *</label>
                            <div className="select-wrapper">
                                <select value={entry.type} onChange={(e) => setEntry({ ...entry, type: e.target.value })} className="form-select">
                        ...
                        <div className="components-editor">
                            ...
                            {entry.components.map((comp, i) => (
                                <div key={i} className="component-row-editor">
                                    <input type="text" placeholder="e.g. RO Membrane, Filter" value={comp.name} onChange={(e) => updateComponent(i, 'name', e.target.value)} className="comp-input comp-name-input" />
                                    <input type="number" placeholder="0" value={comp.price} onChange={(e) => updateComponent(i, 'price', e.target.value)} className="comp-input comp-price-input" min="0" />
                                    <button type="button" onClick={() => removeComponent(i)} className="btn-remove-comp"><Trash2 size={14} /></button>
                                </div>
                            ))}
                            <button type="button" onClick={addComponent} className="btn-add-comp"><Plus size={14} /> Add Part</button>
                        </div>
                    ...
                    <div className="form-field">
                        ...
                        <input type="text" placeholder="Any remarks about this service" value={entry.notes} onChange={(e) => setEntry({ ...entry, notes: e.target.value })} className="form-input" />
                    </div>

                    <div className="form-field">
                        <label>Next Service After (months)</label>
                        <input type="number" placeholder="e.g. 6, 12" value={entry.nextServiceAfterMonths} onChange={(e) => setEntry({ ...entry, nextServiceAfterMonths: e.target.value })} className="form-input" min="0" max="120" />
