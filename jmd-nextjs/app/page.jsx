...
import { formatDate, getNextServiceDate, getUrgencyColor } from '@/lib/utils/helpers';
...
export default function Dashboard() {
    ...
    const toggleSelectAll = () => {
        if (selected.length === pendingCustomers.length) setSelected([]);
        else setSelected(pendingCustomers.map((c) => c._id));
    };

    const handleSendReminders = async () => {
        if (selected.length === 0) { toast.error('Please select at least one customer'); return; }
                ...
                <>
                    <div className="select-all-bar">
                        <button className="btn-select-all" onClick={toggleSelectAll}>
                            {selected.length === pendingCustomers.length ? <CheckSquare size={18} /> : <Square size={18} />}
                    ...
                    <div className="pending-cards">
                        {pendingCustomers.map((customer) => {
                            ...
                            return (
                                ...
                                >
                                    <button className="card-checkbox" onClick={() => toggleSelect(customer._id)}>
                                        {isSelected ? <CheckSquare size={20} color="#60a5fa" /> : <Square size={20} />}
                                    </button>
                                    <div className="card-content" onClick={() => setOpenCard(customer)}>
                                        ...
                                        <div className="card-bottom">
                                            ...
                                            <div className="card-entries-count">{customer.serviceEntries?.length || 0} records</div>
                                        </div>
                                    </div>
                                    <button
            ...
            {selected.length > 0 && (
                <div className="action-bar">
                    <div className="action-bar-inner">
                        ...
                        <button className="btn-send-reminder" onClick={handleSendReminders} disabled={sendingReminder}>
                            <Send size={18} />
                ...
                <div className="modal-overlay" onClick={() => setIgnoreModal({ open: false, customerId: null })}>
                    <div className="modal-box" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="ignore-presets">
                            {[3, 7, 14, 30].map((d) => (
                                <button key={d} className={`preset-btn ${ignoreDays === d ? 'active' : ''}`} onClick={() => setIgnoreDays(d)}>{d}d</button>
                            ))}
                        </div>
                        <div className="ignore-input-row">
                            <input type="number" value={ignoreDays} min={1} max={365} onChange={(e) => setIgnoreDays(Number(e.target.value))} className="ignore-input" />
                            <span>days</span>
                        </div>
                        <div className="modal-actions">
                            <button className="btn-modal-cancel" onClick={() => setIgnoreModal({ open: false, customerId: null })}>Cancel</button>
                            <button className="btn-modal-confirm" onClick={handleIgnoreSubmit}>Snooze</button>
                        </div>
                    </div>
                </div>
            )}
